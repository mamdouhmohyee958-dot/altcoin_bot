...
BTC_BEARISH_STRONG    = -1.0   # 4h change < -1% → رفض كل الإشارات
BTC_BEARISH_LIGHT_MIN = -1.0   # 4h between -0.5% and -1% → تحذير
BTC_BEARISH_LIGHT_MAX = -0.5
BTC_HIGH_ATR_PCT      = 2.0    # ATR >= 2% → تحذير
BTC_CRASH_1H          = -0.5   # 1h < -0.5% → إيقاف فوري
BTC_RECOVERY_1H       = 0.0    # 1h >= 0% → استئناف تلقائي (فوق حد الإيقاف)
...
# --- Spread Filter ---
SPREAD_MAX_PCT           = 0.005   # spread > 0.5% = سيولة ضعيفة → رفض
...
# الشرط 15 (من v6) — Sustained Buy Pressure
PUMP_W_SUSTAINED_BUY     = 3         # 15) Sustained Buy Pressure
...
# ───── الشروط الصارمة (v11 Pro) ─────
STRICT_MODE              = True      # وضع الدقة العالية
...
async def fetch_btc_status(session):
    ...
    """
    try:
        kl1h = await fetch_klines(session, "BTC", interval="1h", limit=25)
        kl4h = await fetch_klines(session, "BTC", interval="4h", limit=16)
    except Exception:
        return {"status": "UNKNOWN", "warning": None, "change_1h": 0, "change_4h": 0, "change_24h": 0, "atr_pct": 0}
    ...
    # تغيّر 24h (آخر 24 شمعة 1h)
    change_24h = 0.0
    if kl1h and len(kl1h) >= 25:
        c_now = kl1h[-1]["close"]
        c_24  = kl1h[-25]["close"]
        if c_24 > 0:
            change_24h = (c_now - c_24) / c_24 * 100
...
def eval_sustained_buy_pressure(candles_1h):
    """
    الشرط 15 (من v6) — Sustained Buy Pressure (max 3 pts)
    3 شموع 1h متتالية خضراء قوية = ضغط شراء مستمر (مش لحظي).
    شمعة قوية = خضراء + جسمها > 50% من المدى + حركة > 0.2%
    """
...
async def evaluate_pump_signal(session, symbol, current_price, volume_24h=0):
        ...
        return {
            ...
            "conditions": {k: {"pass": False, "score": 0, "value": 0, "label": "wash"} for k in
                ["funding_rate","cvd_divergence","taker_buy_ratio","ob_imbalance",
                 ...
                 "short_liq","first_3min","early_surge","vol_price_conf","buysell_press","sustained_buy"]},
    ...
    r15 = eval_sustained_buy_pressure(kl1h)
    ...
    total = (r1["score"] + r2["score"] + r3["score"] + r4["score"] + r5["score"]
             ...
             + r11["score"] + r12["score"] + r13["score"] + r14["score"] + r15["score"])
    ...
    # السعر فوق VWAP (تأكيد إضافي — مش إلزامي مع الفوليم)
    above_vwap, vwap_dist = check_above_vwap(kl1h)
    # التأكيد النهائي = فوليم/شراء قوي (الأساسي). VWAP بونص بيظهر بس مش بيمنع.
    confirmed = vol_confirmed
    if confirmed:
        vwap_note = f" + فوق VWAP +{vwap_dist:.2f}%" if above_vwap else ""
        confirm_reason = f"{vol_reason}{vwap_note}"
    else:
...
async def check_signals(bot: Bot, target_chat: int = None):
    ...
    # 4️⃣ فلترة صارمة: شروط الدقة العالية
    all_results     = [r for r in results if r and not isinstance(r, Exception)]

    def passes_strict(r):
        # 1) لازم confirmed (فوليم/شراء قوي + فوق VWAP)
        if CONFIRM_REQUIRED and not r.get("confirmed", False):
            return False
        if not STRICT_MODE:
            return r["strength"] in ("STRONG", "MODERATE") and r.get("score_pct", 0) >= MIN_SCORE
        # 2) الوضع الصارم
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🔒 الوضع الصارم: {'نشط ✅' if STRICT_MODE else 'متوقف'}\n"
